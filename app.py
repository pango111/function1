...
def load_model():
    ...
    try:
        ...
        # Try to load AI skill explainer - 修复导入路径
        try:
            logger.info("🔄 Loading SkillExplainer...")
            
            # 方法1: 从 models 目录导入（修复后的路径）
            try:
                from models.skill_explainer import SkillExplainer
                skill_explainer = SkillExplainer()
                logger.info(f"✅ SkillExplainer loaded from models directory, available: {skill_explainer.is_available()}")
            except ImportError as e1:
                logger.warning(f"⚠️  Failed to import from models directory: {e1}")
                
                # 方法2: 直接从 skill_explainer 导入
                try:
                    from skill_explainer import SkillExplainer
                    skill_explainer = SkillExplainer()
                    logger.info(f"✅ SkillExplainer loaded directly, available: {skill_explainer.is_available()}")
                except ImportError as e2:
                    logger.warning(f"⚠️  Failed to import directly: {e2}")
                    raise ImportError("Could not import SkillExplainer from any location")
            
            if skill_explainer and skill_explainer.is_available():
                logger.info("✅ AI skill explainer loaded and ready")
            else:
                logger.warning("⚠️  AI skill explainer loaded but OpenAI API not available")
                
        except Exception as e:
            logger.error(f"❌ Failed to load skill explainer: {e}")
...
if __name__ == "__main__":
    ...
    # 如果是通过 gunicorn 启动，也要确保初始化
    print("🔄 Initializing via gunicorn...")
    load_model()
    if skill_explainer:
        print(f"✅ Skill explainer loaded via gunicorn: {skill_explainer.is_available()}")
    else:
        print("❌ Skill explainer failed to load via gunicorn")
